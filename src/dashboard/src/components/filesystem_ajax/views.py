...
def preview_by_uuid(request, uuid):
    """Simple wrapper to the download_by_uid function to create a route
    through the code to enable the preview of a file in the browser, not
    automatically download.
    """
    return download_by_uuid(request, uuid, True)
...
def download_by_uuid(request, uuid, preview_file=False):
    ...
    Returns 404 if a file with the requested UUID cannot be found, and
    400 if the storage service fails to retrieve the record.

    preview_file is an instruction to be applied to the response headers to
    enable the file to be seen inside the browser if it is capable of being
    ...
    response_err_message = 'Storage service returned {}; check logs?'
    return helpers.stream_file_from_storage_service(redirect_url,
